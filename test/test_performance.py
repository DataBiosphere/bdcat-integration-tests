...
class TestPerformance(unittest.TestCase):
    def test_large_pfb_handoff_from_gen3_to_terra(self):
        ...
        with self.subTest('Create a terra workspace.'):
            response = test.utils.create_terra_workspace(workspace=workspace_name)
        ...
        with self.subTest('Import static pfb into the terra workspace.'):
            response = test.utils.import_pfb(workspace=workspace_name, pfb_file=large_pfb)
            self.assertTrue('jobId' in response)

        with self.subTest('Check on the import static pfb job status.'):
            response = test.utils.pfb_job_status_in_terra(workspace=workspace_name, job_id=response['jobId'])
            # this should take < ??????????
        ...
        with self.subTest('Delete the terra workspace.'):
            response = test.utils.delete_terra_workspace(workspace=workspace_name)
            if not response.ok:
                raise RuntimeError(
